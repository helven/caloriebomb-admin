...
class OauthController extends Controller
{
    public function login(Request $request)
    {
        ...
        $complex_name = 'unknown_device';
        if (strpos($user_agent, 'Mozilla') !== false) {
            if (strpos($user_agent, 'Chrome') !== false) {
                $complex_name = 'chrome' . substr($device_fingerprint, 0, 8);
            } elseif (strpos($user_agent, 'Firefox') !== false) {
                $complex_name = 'firefox' . substr($device_fingerprint, 0, 8);
            } else {
                $complex_name = 'browser' . substr($device_fingerprint, 0, 8);
            }
        } elseif (strpos($user_agent, 'PostmanRuntime') !== false) {
            $complex_name = 'postman' . substr($device_fingerprint, 0, 8);
        }

        $device_name = $request->uuid ?? $complex_name;
